...
local hometab = Window:CreateTab("Инфа")
local farmtab = Window:CreateTab("Сбор")
...
local extrtab = Window:CreateTab("Дополн")
...
information:CreateLabel("⚠️ - Небезопасная функция")
information:CreateLabel("⚙ - Настраиваемая функция")
information:CreateLabel("Скрипт перевел BateRik753")
local gainedhoneylabel = information:CreateLabel("Gained Honey: 0")
information:CreateButton("Discord чат", function() setclipboard("https://discord.gg/7ZwzbCNtHx") end)
information:CreateButton("Донат на бургер", function() setclipboard("https://donatepay.ru/don/BateRik753") end)
information:CreateLabel("")
information:CreateLabel("Скрипт будет обновляться")
information:CreateLabel("")
local farmo = farmtab:CreateSection("Сбор пыльцу")
local fielddropdown = farmo:CreateDropdown("Поле для сбора", fieldstable, function(String) kocmoc.vars.field = String end) fielddropdown:SetOption(fieldstable[1])
convertatslider = farmo:CreateSlider("Конверт пыльцу в мед при % ", 0, 100, 100, false, function(Value) kocmoc.vars.convertat = Value end)
local autofarmtoggle = farmo:CreateToggle("Автофарм ⚙", nil, function(State) kocmoc.toggles.autofarm = State end) autofarmtoggle:CreateKeybind("U", function(Key) end)
farmo:CreateToggle("Увеличение сбора", nil, function(State) kocmoc.toggles.autodig = State end)
farmo:CreateDropdown("Режим собирания", {"Нормальный","Колективно воровать"}, function(Option)  kocmoc.vars.autodigmode = Option end)
farmo:CreateToggle("Не преобразовывать пыльцу", nil, function(State) kocmoc.toggles.disableconversion = State end)
farmo:CreateToggle("Авто спринклер", nil, function(State) kocmoc.toggles.autosprinkler = State end)
farmo:CreateToggle("Собирать Пузыри", nil, function(State) kocmoc.toggles.farmbubbles = State end)
farmo:CreateToggle("Собирать Пламя", nil, function(State) kocmoc.toggles.farmflame = State end)
farmo:CreateToggle("Собирать падающие Кокосы", nil, function(State) kocmoc.toggles.farmcoco = State end)
farmo:CreateToggle("Собирать Точное перекрестие", nil, function(State) kocmoc.toggles.collectcrosshairs = State end)
farmo:CreateToggle("Собирать Fuzzy Бомбы", nil, function(State) kocmoc.toggles.farmfuzzy = State end)
farmo:CreateToggle("Собирать воздушные шары", nil, function(State) kocmoc.toggles.farmunderballoons = State end)
farmo:CreateToggle("Собирать Под облаками", nil, function(State) kocmoc.toggles.farmclouds = State end)
--farmo:CreateToggle("Farm Closest Leaves", nil, function(State) kocmoc.toggles.farmclosestleaf = State end)

local farmt = farmtab:CreateSection("Сбор подарков, предметов")
farmt:CreateToggle("Авто Диспенсер ⚙", nil, function(State) kocmoc.toggles.autodispense = State end)
farmt:CreateToggle("Авто Буст полей ⚙", nil, function(State) kocmoc.toggles.autoboosters = State end)
farmt:CreateToggle("Авто Часы богатства", nil, function(State) kocmoc.toggles.clock = State end)
farmt:CreateToggle("Авто Пряничные мишки", nil, function(State) kocmoc.toggles.collectgingerbreads = State end)
farmt:CreateToggle("Авто Самовар", nil, function(State) kocmoc.toggles.autosamovar = State end)
farmt:CreateToggle("Авто Чулки", nil, function(State) kocmoc.toggles.autostockings = State end)
farmt:CreateToggle("Авто Плантерс", nil, function(State) kocmoc.toggles.autoplanters = State end):AddToolTip("Пересадит ваши плантаторы после преобразования, если они достигнут 100%")
farmt:CreateToggle("Авто Медовые свечи", nil, function(State) kocmoc.toggles.autocandles = State end)
farmt:CreateToggle("Авто Пчелиный праздник", nil, function(State) kocmoc.toggles.autofeast = State end)
farmt:CreateToggle("Авто Подарки от Онета", nil, function(State) kocmoc.toggles.autoonettart = State end)
farmt:CreateToggle("Авто Бесплатные муравиные пропуска", nil, function(State) kocmoc.toggles.freeantpass = State end)
farmt:CreateToggle("Авто Ростки", nil, function(State) kocmoc.toggles.farmsprouts = State end)
farmt:CreateToggle("Авто сбор Грибов", nil, function(State) kocmoc.toggles.farmpuffshrooms = State end)
farmt:CreateToggle("Авто сбор Снежинок ⚠️", nil, function(State) kocmoc.toggles.farmsnowflakes = State end)
farmt:CreateToggle("Телепорт к Редким ⚠️", nil, function(State) kocmoc.toggles.farmrares = State end)
farmt:CreateToggle("Авто принятие/подтверждение квестов ⚙", nil, function(State) kocmoc.toggles.autoquest = State end)
farmt:CreateToggle("Автоматически выполнять квесты ⚙", nil, function(State) kocmoc.toggles.autodoquest = State end)
farmt:CreateToggle("Авто Медовый шторм", nil, function(State) kocmoc.toggles.honeystorm = State end)


local mobkill = combtab:CreateSection("Кого убивать")
mobkill:CreateToggle("Убить Краба", nil, function(State) if State then api.humanoidrootpart().CFrame = CFrame.new(-307.52117919922, 107.91863250732, 467.86791992188) end end)
mobkill:CreateToggle("Убить Улитку", nil, function(State) fd = game.Workspace.FlowerZones['Stump Field'] if State then api.humanoidrootpart().CFrame = CFrame.new(fd.Position.X, fd.Position.Y-6, fd.Position.Z) else api.humanoidrootpart().CFrame = CFrame.new(fd.Position.X, fd.Position.Y+2, fd.Position.Z) end end)
mobkill:CreateToggle("Убить Мондо", nil, function(State) kocmoc.toggles.killmondo = State end)
mobkill:CreateToggle("Искать и убить Шипастую пчелу", nil, function(State) kocmoc.toggles.killvicious = State end)
mobkill:CreateToggle("Искать и убить Ветряную пчелу", nil, function(State) kocmoc.toggles.killwindy = State end)
mobkill:CreateToggle("Авто убивать мобов", nil, function(State) kocmoc.toggles.autokillmobs = State end):AddToolTip("Убивает мобов после x преобразования пыльцы")
mobkill:CreateToggle("Избегать мобов", nil, function(State) kocmoc.toggles.avoidmobs = State end)
mobkill:CreateToggle("Авто Муравей", nil, function(State) kocmoc.toggles.autoant = State end):AddToolTip("Вам нужны искры 😋; Идет после Муравиного Испытание и конвертации пыльцы")

local serverhopkill = combtab:CreateSection("Переключение между серверами")
serverhopkill:CreateButton("Искать сервер с шыпастой пчелой ⚠️",function() loadstring(game:HttpGet("https://raw.githubusercontent.com/Boxking776/kocmoc/main/functions/viciousbeeserverhop.lua"))() end):AddToolTip("Серверные прыжки для Шыпастой пчелы")
serverhopkill:CreateButton("Искать сервер с ветряной пчелой ⚠️",function() loadstring(game:HttpGet("https://raw.githubusercontent.com/Boxking776/kocmoc/main/functions/windybeeserverhop.lua"))() end):AddToolTip("Серверные прыжки для Ветряной пчелы")
serverhopkill:CreateLabel("")
serverhopkill:CreateLabel("[⚠️] Эти функции разгружают UI")
serverhopkill:CreateLabel("")

local amks = combtab:CreateSection("Настройки автоматического убийства мобов")
amks:CreateTextBox('Убить мобов после x Конверсий', 'default = 3', true, function(Value) kocmoc.vars.monstertimer = tonumber(Value) end)


local wayp = wayptab:CreateSection("Waypoints")
wayp:CreateDropdown("Телепорт к полю", fieldstable, function(Option) game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = game:GetService("Workspace").FlowerZones:FindFirstChild(Option).CFrame end)
wayp:CreateDropdown("Телепорт к монстрам", spawnerstable, function(Option) d = game:GetService("Workspace").MonsterSpawners:FindFirstChild(Option) game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(d.Position.X, d.Position.Y+3, d.Position.Z) end)
wayp:CreateDropdown("Телепорт к игрушкам", toystable, function(Option) d = game:GetService("Workspace").Toys:FindFirstChild(Option).Platform game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(d.Position.X, d.Position.Y+3, d.Position.Z) end)
wayp:CreateButton("Телепорт к улью", function() game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = game:GetService("Players").LocalPlayer.SpawnPos.Value end)

local useitems = itemstab:CreateSection("Использовать предметы")

useitems:CreateButton("Используй все баффы ⚠️",function() for i,v in pairs(buffTable) do  game:GetService("ReplicatedStorage").Events.PlayerActivesCommand:FireServer({["Name"]=v}) end end)
...
miscc:CreateButton("Полубог в Муравиное испытание", function() api.tween(1, CFrame.new(93.4228, 32.3983, 553.128)) task.wait(1) game.ReplicatedStorage.Events.ToyEvent:FireServer("Ant Challenge") game.Players.LocalPlayer.Character.HumanoidRootPart.Position = Vector3.new(93.4228, 42.3983, 553.128) task.wait(2) game.Players.LocalPlayer.Character.Humanoid.Name = 1 local l = game.Players.LocalPlayer.Character["1"]:Clone() l.Parent = game.Players.LocalPlayer.Character l.Name = "Humanoid" task.wait() game.Players.LocalPlayer.Character["1"]:Destroy() api.tween(1, CFrame.new(93.4228, 32.3983, 553.128)) task.wait(8) api.tween(1, CFrame.new(93.4228, 32.3983, 553.128)) end)
local wstoggle = miscc:CreateToggle("Скорость ходьбы", nil, function(State) kocmoc.toggles.loopspeed = State end) wstoggle:CreateKeybind("K", function(Key) end)
local jptoggle = miscc:CreateToggle("Сила прыжка", nil, function(State) kocmoc.toggles.loopjump = State end) jptoggle:CreateKeybind("L", function(Key) end)
miscc:CreateToggle("Режим бога", nil, function(State) kocmoc.toggles.godmode = State if State then bssapi:Godmode(true) else bssapi:Godmode(false) end end)
local misco = misctab:CreateSection("Другое")
misco:CreateDropdown("Надеть аксессуары", accesoriestable, function(Option) local ohString1 = "Equip" local ohTable2 = { ["Mute"] = false, ["Type"] = Option, ["Category"] = "Accessory" } game:GetService("ReplicatedStorage").Events.ItemPackageEvent:InvokeServer(ohString1, ohTable2) end)
misco:CreateDropdown("Экипировать маски", masktable, function(Option) local ohString1 = "Equip" local ohTable2 = { ["Mute"] = false, ["Type"] = Option, ["Category"] = "Accessory" } game:GetService("ReplicatedStorage").Events.ItemPackageEvent:InvokeServer(ohString1, ohTable2) end)
misco:CreateDropdown("Экипировать коллекционеров", collectorstable, function(Option) local ohString1 = "Equip" local ohTable2 = { ["Mute"] = false, ["Type"] = Option, ["Category"] = "Collector" } game:GetService("ReplicatedStorage").Events.ItemPackageEvent:InvokeServer(ohString1, ohTable2) end)
...
misco:CreateButton("Экспорт таблицы статистики", function() local StatCache = require(game.ReplicatedStorage.ClientStatCache)writefile("Stats_"..api.nickname..".json", StatCache:Encode()) end)

local autofeed = itemstab:CreateSection("Автокорм каждой из пчел")
...
autofeed:CreateDropdown("Выберите чем кормить",treatsTable,function(option) kocmoc.vars.selectedTreat = option end)
autofeed:CreateTextBox("Количество корма для каждой пчелы","10",false,function(Value) kocmoc.vars.selectedTreatAmount = tonumber(Value) end)
autofeed:CreateButton("Накормить всех пчел",function() feedAllBees(kocmoc.vars.selectedTreat,kocmoc.vars.selectedTreatAmount) end)

local windShrine = itemstab:CreateSection("Храм Ветра")
windShrine:CreateDropdown("Выбрать предмет", donatableItemsTable, function(Option)  kocmoc.vars.donoItem = Option end)
windShrine:CreateTextBox("Количество товара","10",false,function(Value) kocmoc.vars.donoAmount = tonumber(Value) end)
...
windShrine:CreateToggle("Авто пожертвовать",nil,function(selection)
    kocmoc.toggles.autodonate = selection
end)
local extras = extrtab:CreateSection("Для слабых ПК")
extras:CreateButton("Увеличение FPS", function()loadstring(game:HttpGet("https://raw.githubusercontent.com/Boxking776/kocmoc-bss-archive/main/functions/boostfps.lua"))()end)
extras:CreateButton("Уничтожить наклейки", function()
    local g = game
...
extras:CreateTextBox("Скорость планера", "", true, function(Value) local StatCache = require(game.ReplicatedStorage.ClientStatCache) local stats = StatCache:Get() stats.EquippedParachute = "Glider" local module = require(game:GetService("ReplicatedStorage").Parachutes) local st = module.GetStat local glidersTable = getupvalues(st) glidersTable[1]["Glider"].Speed = Value setupvalue(st, st[1]'Glider', glidersTable) end)
extras:CreateTextBox("Планер Поплавок", "", true, function(Value) local StatCache = require(game.ReplicatedStorage.ClientStatCache) local stats = StatCache:Get() stats.EquippedParachute = "Glider" local module = require(game:GetService("ReplicatedStorage").Parachutes) local st = module.GetStat local glidersTable = getupvalues(st) glidersTable[1]["Glider"].Float = Value setupvalue(st, st[1]'Glider', glidersTable) end)
extras:CreateButton("Невидимость", function(State) api.teleport(CFrame.new(0,0,0)) wait(1) if game.Players.LocalPlayer.Character:FindFirstChild('LowerTorso') then Root = game.Players.LocalPlayer.Character.LowerTorso.Root:Clone() game.Players.LocalPlayer.Character.LowerTorso.Root:Destroy() Root.Parent = game.Players.LocalPlayer.Character.LowerTorso api.teleport(game:GetService("Players").LocalPlayer.SpawnPos.Value) end end)
extras:CreateToggle("Плавать", nil, function(State) temptable.float = State end)


local farmsettings = setttab:CreateSection("Настройки автосбора")
farmsettings:CreateTextBox("Собирать скорость ходьбы", "Default Value = 60", true, function(Value) kocmoc.vars.farmspeed = Value end)
farmsettings:CreateToggle("^ Цикл скорости при автоматическом фарме",nil, function(State) kocmoc.toggles.loopfarmspeed = State end)
farmsettings:CreateToggle("Не ходите в поле",nil, function(State) kocmoc.toggles.farmflower = State end)
farmsettings:CreateToggle("Преобразование воздушного шара улья",nil, function(State) kocmoc.toggles.convertballoons = State end)
farmsettings:CreateToggle("Не фармите жетоны",nil, function(State) kocmoc.toggles.donotfarmtokens = State end)
farmsettings:CreateToggle("Включить черный список токенов",nil, function(State) kocmoc.toggles.enabletokenblacklisting = State end)
farmsettings:CreateSlider("Скорость ходьбы", 0, 120, 70, false, function(Value) kocmoc.vars.walkspeed = Value end)
farmsettings:CreateSlider("Сила прыжка", 0, 120, 70, false, function(Value) kocmoc.vars.jumppower = Value end)
local raresettings = setttab:CreateSection("Настройки токенов")
raresettings:CreateTextBox("Идентификатор объекта", 'rbxassetid', false, function(Value) rarename = Value end)
raresettings:CreateButton("Добавить токен в список редких", function()
    ...
    raresettings:CreateDropdown("Список редких", kocmoc.rares, function(Option) end)
end)
raresettings:CreateButton("Удалить токен из списка редких", function()
...
raresettings:CreateButton("Добавить токен в черный список", function()
    ...
    raresettings:CreateDropdown("Черный список токенов", kocmoc.bltokens, function(Option) end)
end)
...
raresettings:CreateDropdown("Черный список токенов", kocmoc.bltokens, function(Option) end)
raresettings:CreateDropdown("Список редких", kocmoc.rares, function(Option) end)
local dispsettings = setttab:CreateSection("Настройки автодиспенсера и автобустеров")
dispsettings:CreateToggle("Диспенсер маточного молочка/Royal Jelly", nil, function(State) kocmoc.dispensesettings.rj = not kocmoc.dispensesettings.rj end)
dispsettings:CreateToggle("Диспенсер для черники/Blueberry", nil,  function(State) kocmoc.dispensesettings.blub = not kocmoc.dispensesettings.blub end)
dispsettings:CreateToggle("Диспенсер для клубники/Strawberry", nil,  function(State) kocmoc.dispensesettings.straw = not kocmoc.dispensesettings.straw end)
dispsettings:CreateToggle("Диспенсер для лечения", nil,  function(State) kocmoc.dispensesettings.treat = not kocmoc.dispensesettings.treat end)
dispsettings:CreateToggle("Кокосовый Диспенсер", nil,  function(State) kocmoc.dispensesettings.coconut = not kocmoc.dispensesettings.coconut end)
dispsettings:CreateToggle("Дозатор клея", nil,  function(State) kocmoc.dispensesettings.glue = not kocmoc.dispensesettings.glue end)
dispsettings:CreateToggle("Ускоритель на вершине горы", nil,  function(State) kocmoc.dispensesettings.white = not kocmoc.dispensesettings.white end)
dispsettings:CreateToggle("Усилитель синего поля", nil,  function(State) kocmoc.dispensesettings.blue = not kocmoc.dispensesettings.blue end)
dispsettings:CreateToggle("Усилитель красного поля", nil,  function(State) kocmoc.dispensesettings.red = not kocmoc.dispensesettings.red end)
...
kocmocs:CreateTextBox("Имя конфигурации", 'ex: stumpconfig', false, function(Value) temptable.configname = Value end)
kocmocs:CreateButton("Загрузить конфигурацию", function() kocmoc = game:service'HttpService':JSONDecode(readfile("kocmoc/BSS_"..temptable.configname..".json")) end)
kocmocs:CreateButton("Сохранить конфигурацию", function() writefile("kocmoc/BSS_"..temptable.configname..".json",game:service'HttpService':JSONEncode(kocmoc)) end)
kocmocs:CreateButton("Сбросить конфигурацию", function() kocmoc = defaultkocmoc end)
local fieldsettings = setttab:CreateSection("Настройки полей")
fieldsettings:CreateDropdown("Лучшее белое поле", temptable.whitefields, function(Option) kocmoc.bestfields.white = Option end)
fieldsettings:CreateDropdown("Лучшее красное поле", temptable.redfields, function(Option) kocmoc.bestfields.red = Option end)
fieldsettings:CreateDropdown("Лучшее голубое поле", temptable.bluefields, function(Option) kocmoc.bestfields.blue = Option end)
fieldsettings:CreateDropdown("Поле", fieldstable, function(Option) temptable.blackfield = Option end)
fieldsettings:CreateButton("Добавить поле в черный список", function() table.insert(kocmoc.blacklistedfields, temptable.blackfield) game:GetService("CoreGui"):FindFirstChild(_G.windowname).Main:FindFirstChild("Blacklisted Fields D",true):Destroy() fieldsettings:CreateDropdown("Blacklisted Fields", kocmoc.blacklistedfields, function(Option) end) end)
fieldsettings:CreateButton("Удалить поле из черного списка", function() table.remove(kocmoc.blacklistedfields, api.tablefind(kocmoc.blacklistedfields, temptable.blackfield)) game:GetService("CoreGui"):FindFirstChild(_G.windowname).Main:FindFirstChild("Blacklisted Fields D",true):Destroy() fieldsettings:CreateDropdown("Blacklisted Fields", kocmoc.blacklistedfields, function(Option) end) end)
fieldsettings:CreateDropdown("Поля из черного списка", kocmoc.blacklistedfields, function(Option) end)
local aqs = setttab:CreateSection("Настройки автоквеста")
aqs:CreateDropdown("Выполняйте квесты NPC", {'Все квесты', 'Буко Би/Bucko Bee', 'Бурый медведь/Brown Bear', 'Райли Би/Riley Bee', 'Белый медведь/Polar Bear'}, function(Option) kocmoc.vars.npcprefer = Option end)
aqs:CreateToggle("Телепортироваться к NPC", nil, function(State) kocmoc.toggles.tptonpc = State end)
local pts = setttab:CreateSection("Приоритетные жетоны автосбора")
pts:CreateTextBox("Идентификатор объекта", 'rbxassetid', false, function(Value) rarename = Value end)
pts:CreateButton("Добавить токен в список приоритетов", function() table.insert(kocmoc.priority, rarename) game:GetService("CoreGui"):FindFirstChild(_G.windowname).Main:FindFirstChild("Priority List D",true):Destroy() pts:CreateDropdown("Priority List", kocmoc.priority, function(Option) end) end)
pts:CreateButton("Удалить токен из списка приоритетов", function() table.remove(kocmoc.priority, api.tablefind(kocmoc.priority, rarename)) game:GetService("CoreGui"):FindFirstChild(_G.windowname).Main:FindFirstChild("Priority List D",true):Destroy() pts:CreateDropdown("Priority List", kocmoc.priority, function(Option) end) end)
pts:CreateDropdown("Список приоритетов", kocmoc.priority, function(Option) end)
